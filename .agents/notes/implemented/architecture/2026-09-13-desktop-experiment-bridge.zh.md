...
后端管道清理销毁各 Node 流并等待 close 事件。即使设置 `autoClose: false`，销毁流也会关闭其描述符；另行关闭描述符会与该清理竞争。完整请求管道的 EOF 也会启动正常关闭，因为它可能先于独立 IPC 通道上的退出消息到达；截断帧仍作为传输错误处理。发布冒烟检查要求正常子进程以零状态退出，验证离线关闭及重新开启，并强制完整和截断 EOF 先于任何 IPC 退出消息到达。每个阶段都有独立的有限超时。

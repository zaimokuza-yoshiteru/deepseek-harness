...
Windows 上原先的首次启动流程在 pnpm 开始安装前花了 154 秒准备 store，冒烟上限打断了仍在推进的安装。持久化 store 尚不存在时，现在通过桌面数据根目录内的重命名直接采用已验证的解压结果，省去约 21,000 个文件的再次复制和删除。已有 store 仍执行合并，保留升级时的插件文件和包索引记录。

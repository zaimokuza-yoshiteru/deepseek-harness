...
vi.mock('../src/shell-environment.ts', () => ({ desktopShellEnvironment: async (env: NodeJS.ProcessEnv) => env }))
vi.mock('../src/distribution.ts', () => ({ configureDesktopDistribution: () => 'desktop-test-data' }))
vi.mock('node:fs', async importOriginal => ({ ...await importOriginal<typeof import('node:fs')>(), realpathSync: (path: string) => path }))

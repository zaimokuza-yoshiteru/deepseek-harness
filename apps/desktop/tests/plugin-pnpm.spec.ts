...
it('installs a real pnpm graph, then executes approved scripts with the shared host instance', async () => {
  ...
  try {
    ...
    writeFileSync(pnpm, `process.argv = process.argv.filter(arg => !arg.startsWith('--config.userconfig=')); process.argv.splice(2, 0, ${JSON.stringify(`--config.registry=${origin}`)}, ${JSON.stringify(`--config.userconfig=${config}`)}); await import(${JSON.stringify(pathToFileURL(realPnpm).href)})`)

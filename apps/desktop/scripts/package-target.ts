...
async function main(): Promise<void> {
  ...
  const environment = invocation.portable ? { ...process.env } : loadDesktopPackageEnvironment(target.platform)
  ...
  const buildVersion = invocation.portable ? productVersion : await resolveRequestedBuildVersion(invocation, productVersion, environment)
  ...
  let success = false
  try {
    if (!invocation.portable) await packagingStep(run.directory, 'configuration', async () => { validateDesktopPackageEnvironment(environment, target, invocation) }, secrets)
    if (!invocation.portable) await packagingStep(run.directory, 'toolchain', () => requireDesktopToolchain(target.platform, environment), secrets)
    if (target.platform === 'darwin' && !invocation.portable) {
...
export async function packageTarget(
  ...
  const electronBuilderEnv = desktopElectronBuilderEnvironment(downloadEnv, invocation.unsigned || invocation.portable === true)
  for (const name of WINDOWS_SIGNING_ENV_NAMES) {
    if (!invocation.unsigned && !invocation.portable && environment[name] !== undefined) electronBuilderEnv[name] = environment[name]
  }
  const signPrimaryRuntime = !invocation.portable && target.platform === 'win32' && !invocation.unsigned && !invocation.prepareOnly
  ...
  if (invocation.portable) await execute(['run', 'build'], targetEnv)
  if (invocation.prepareOnly) return
  if (invocation.portable) {
    const args = desktopElectronBuilderArguments(target, invocation.directory)
      .map(arg => arg === 'electron-builder.config.mjs' ? 'electron-builder.portable.config.mjs' : arg)
    await execute(args, electronBuilderEnv)
    return
  }

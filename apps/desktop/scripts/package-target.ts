...
async function main(): Promise<void> {
  ...
  if (!invocation.portable) validateDesktopPackageEnvironment(environment, target, invocation)
  if (invocation.check) {
...
export async function packageTarget(
  ...
  const electronBuilderEnv = desktopElectronBuilderEnvironment(targetEnv, invocation.unsigned || invocation.portable === true)

...
便携标签采用 `0.1.7.alpha.2.<counter>`，应用元数据采用 `0.1.7-alpha.2.<counter>`。核心与插件版本分别固定。标准上游安装包命令从 package manifest 推导版本。

...
This branch targets macOS Apple Silicon and Windows x64 ZIPs with DSH `0.1.7-rc.1`, ACP adapter `0.1.7-rc.1.0`, and Agent Teams Office `0.1.0-beta.5`. Recipients do not install DSH, Node.js, npm, or pnpm separately. Agent executables such as Devin and Kimi remain external.
...
On Windows, use a shallow extraction directory such as `C:\DSH`. The bundled native Office engine can fail to initialize from deeply nested application paths even when all files are present. If document conversion fails after extraction into a deep directory, close the application, move the complete extracted folder to a shorter path, and reopen it. The data home and profiles remain in place.
...
Open **Plugins** in the application sidebar to use DSH’s native plugin manager. Choose its installation action, enter a package specification such as `@scope/plugin-name@version`, review the package, and install it. Plugins must support DSH `0.1.7-rc.1`. The native manager owns package installation, removal and activation, and reports any required reload or blocked operation. Plugin Hub and its desktop IPC bridge are not included.
...
Portable tags use `0.1.7.rc.1.<counter>` and application metadata uses `0.1.7-rc.1.<counter>`. Core and plugin versions remain separately pinned. Standard upstream installer commands derive their versions from the package manifests.

...
const api: DshDesktopApi = {
  ...
  agentTeams: {
    enabled: () => ipcRenderer.invoke(DESKTOP_IPC.agentTeamsGet) as Promise<boolean>,
    setEnabled: enabled => ipcRenderer.invoke(DESKTOP_IPC.agentTeamsSet, enabled) as Promise<void>,
  },
  updates: {

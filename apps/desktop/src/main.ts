...
function createWindow(preload: string, show = false, primary = false): BrowserWindow {
  ...
  window.webContents.on('context-menu', (_event, { isEditable, selectionText, editFlags, linkURL }) => {
    ...
    if (linkURL && /^https?:/u.test(linkURL)) {
      const messages = currentDesktopLocale().messages
      items.push({ label: messages.openLink, click: () => { void shell.openExternal(linkURL) } },
        { label: messages.copyLink, click: () => { void clipboard.writeText(linkURL) } })
    }

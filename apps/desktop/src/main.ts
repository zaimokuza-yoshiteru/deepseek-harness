...
function runtimeResources(): RuntimeResources {
  ...
  const packageNode = join(process.resourcesPath, 'runtime', 'node', process.platform === 'win32' ? 'node.exe' : 'node')
  return { node, pnpm, dsh, packageNode, ...(development ? {} : { profileResolution: 'runtime', pluginSeed: join(process.resourcesPath, 'plugin-seed') }) }
}
...
function createWindow(preload: string, show = false): BrowserWindow {
  ...
  const openExternal = (url: string): void => {
    if (/^https?:/iu.test(url)) void shell.openExternal(url).catch((error: unknown) => { console.error(error) })
  }
  window.webContents.setWindowOpenHandler(({ url }) => { openExternal(url); return { action: 'deny' } })
  window.webContents.on('context-menu', (_event, params) => {
    const items: Electron.MenuItemConstructorOptions[] = []
    if (params.linkURL) items.push({ label: params.linkURL, enabled: false }, {
      label: resolveDesktopLocale(app.getLocale()).messages.copyLink,
      click: () => { void clipboard.writeText(params.linkURL).catch((error: unknown) => { console.error(error) }) },
    })
  ...
  window.webContents.on('will-navigate', (event, url) => {
    if (new URL(url).protocol !== `${SCHEME}:`) { event.preventDefault(); openExternal(url) }
...
async function main(): Promise<void> {
  ...
  let experiments: DesktopExperiments | undefined
  const shellEnvironment = desktopShellEnvironment(process.env, resources.packageNode)
    .then(env => ({ env }), (error: unknown) => ({ error }))
  ...
  const reconcileBackend = (): Promise<void> => {
    startup ??= (async () => {
      ...
      await backend.start(async () => {
        const result = await shellEnvironment
        if ('error' in result) throw result.error
        const ownedHome = process.env.DSH_HOME
        Object.assign(process.env, result.env)
        if (ownedHome === undefined) delete process.env.DSH_HOME
        else process.env.DSH_HOME = ownedHome
        if (development === undefined) {
          await manager.applyRelease()
        }
      })
      if (backend.host !== undefined) {
        if (experiments === undefined && development === undefined) {
          experiments = new DesktopExperiments({
            profile: realpathSync(activeProject), supported: true,
            enabled: () => manager.agentTeamsEnabled(), busy: () => changingProfile,
            setBusy: (value) => { changingProfile = value },
            stopIfIdle: async () => {
              if (backend.host !== undefined && !await backend.host.stopIfIdle()) return false
              await backend.stop()
              return true
            },
            change: enabled => manager.mutate({ type: 'agent-teams', enabled }, hooks),
            recover: () => backend.start(async () => {}),
          })
